...
De igual manera no es posible apreciar una moda, por lo tanto para este conjunto no obtendremos una moda debido a la naturaleza de los datos. Además la moda es más utilizada para variables categóricas.
...
El valor de la asímetria es mayor o igual que -0.5 y menor o igual que 0.5 entonces podemos afirmar que la distribución es aproximadamente símetrica.

El valor dela curtosis es mayor a 0.5 podemos afirmar que los valores de la distribución tienen una gran concentración en torno a la media del conjunto.
...
Los valores del conjunto de datos son categóricos por lo cual es posible obtener su media
...
```{r}
# Seleccionar solo las columnas de datos (sin la columna de categorías)
datos = as.matrix(papas_x_refrescos[, -1])

# Crear mapa de calor
heatmap(papas_x_refrescos,
        Rowv = NA,    # Evitar mostrar dendrograma en filas
        Colv = NA,    # Evitar mostrar dendrograma en columnas
        col = cm.colors(256),  # Colores del mapa de calor
        scale = "none",        # No escalar los datos
        margins = c(5, 10)     # Añadir márgenes
)
```
